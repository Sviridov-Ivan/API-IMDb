...
interface IMDbApiService {
    ...
    suspend fun searchMovies(@Path("expression") expression: String): MoviesSearchResponse // передачи текста поискового запроса
    ...
    //fun searchNames(@Path("expression") expression: String): Call<NamesSearchResponse> // получение имен актеров (использовал до корутин и flow)
    suspend fun searchNames(@Path("expression") expression: String): NamesSearchResponse // suspend, что позволит нам работать с этим запросом в корутинах и других suspend-функциях
}
